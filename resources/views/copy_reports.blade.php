...
<div class="row mb-4 dashboard-header">
    <div class="col-12">
        <h4 class="mb-0">Copy Request Reports</h4>
        <p class="text-muted mb-0">View and filter copy request history</p>
    </div>
</div>
        ...
        <table class="modern-table tables">
            ...
            <tbody>
                @foreach($requests as $request)
                    ...
                    <tr>
                        <td>
                            <span class="reference-no">CR-{{str_pad($request->id, 5, '0', STR_PAD_LEFT)}}</span>
                        </td>
                        <td>{{date('M. d, Y',strtotime($request->created_at))}}</td>
                        <td><strong>{{$request->control_code}}</strong></td>
                        <td>{{$request->title}}</td>
                        <td>{{$request->user->name}}</td>
                        <td>{{date('M. d, Y',strtotime($request->date_needed))}}</td>
                        <td>
                            @if($request->expiration_date != null)
                                {{date('M. d, Y',strtotime($request->expiration_date))}}
                            @else
    ...
    $(document).ready(function(){
        $('.tables').DataTable({
            ...
            buttons: [
                {extend: 'copy'},
                {extend: 'csv'},
                {extend: 'excel', title: 'Copy Request Reports'},
                {extend: 'pdf', title: 'Copy Request Reports'},
                {extend: 'print',
